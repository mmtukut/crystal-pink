...
const sketch = ({ context, canvas, width, height }) => {
  ...
  const hdrEquirect = new THREE.RGBELoader().load(
    ...
    (texture) => {
      ...
      // Rotate the texture by a certain angle (in radians)
      const angleInRadians = Math.PI / 2; // For example, rotate by 45 degrees
      const textureMatrix = new THREE.Matrix4().makeRotationY(angleInRadians);
      texture.matrix = textureMatrix;
  
      // Optionally, you can set the needsUpdate flag to true if the texture needs to be updated
